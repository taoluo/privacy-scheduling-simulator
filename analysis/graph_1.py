...
from .plot import plot_granted_tasks
import sys
# sys.path.append('/foo/bar/mock-0.3.1')
# def _plot_granted_tasks_subgraph(table,columns):
plot_rdp = True
plot_dp = False
def plot_graph1(table,save_file_name):
    ...
    plot_granted_tasks(save_file_name, table1, title % 'granted_tasks_total' ,column_to_plot='granted_tasks_total')

    if should_plot_per_task_type:
        col = 'num_granted_tasks_l_dp_l_blk'
        plot_granted_tasks(save_file_name.split('.')[0]+'-%s.pdf' % col, table1, title % col, column_to_plot=col)
...
# 
# def plot_granted_tasks(save_file_name, table1, title, column_to_plot):
